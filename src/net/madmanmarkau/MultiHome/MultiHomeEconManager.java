...
public class MultiHomeEconManager {
	...
	protected static void initialize(MultiHome plugin) {
		...
		if (iConomy5 != null && Settings.isEconomyEnabled()) {
			...
			Messaging.logWarning(" - Economy enabled using: iConomy v" + version, plugin);
		} else {
			handler = EconomyHandler.NONE;
			Messaging.logWarning(" - An economy plugin wasn't detected or MultiHome is not set to use economy", plugin);
		}
	}
